...
function createBurst(x, y) {
	const numLetters = 15; // letras na explosão
	const numSparkles = 50; // brilho na explosão para dar mais efeito
...
function animateParticle(el, isSparkle) {
	const angle = Math.random() * Math.PI * 2; // direção aleatoria
	const distance = anime.random(100, 200); // acrescentando distância no efeito
	const duration = anime.random(1200, 2000); // duração maior no efeito dramático
	const fallDistance = anime.random(20, 80); // realismo do efeito de cair
	...
	anime
			.timeline({
					...
					easing: "easeOutCubic",
					duration: duration,
					complete: () => el.remove() // Remover o elemento apos a explosão
			})
			.add({
			...
			.add({
					translateY: `+=${fallDistance}px`, // gravidade larga efeito
					opacity: [0.9, 0],
					easing: "easeInCubic",
					duration: duration / 2
			});
